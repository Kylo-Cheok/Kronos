"""Phase 9 R11 — Fix magnitude gate data leakage.

R10 review uncovered that `gate_base` used `|trh1| >= mag` where `trh1` is the
*true* future h1 return — a lookahead leak. Real trading cannot know future
return magnitude. This script replaces it with the model's own h1 return
prediction magnitude (`|pred_ret_h1| >= mag`) and re-evaluates the r5 baseline
across the same gate configs to establish the leak-free alpha.

Also reports the leak-free vs leaky comparison so the inflation from the bug
is quantified.

Output: outputs/eval_p9_panel_r11_leakfix.json
"""
...
# Leak-inflated baselines (from R8/R10 r5 results) for comparison
LEAKY_BASELINES = {
...
def predict_all_horizons(loaded, w, lookbacks):
    """Return per-lb direction logits AND return predictions (for mag gate)."""
    max_lb = max(lookbacks)
    per_lb_logits = []
    per_lb_ret = []
    ...
    return {"per_lb_logits": per_lb_logits, "per_lb_ret": per_lb_ret,
            "target_direction": target_dir,
            "target_return": target_ret}


def gate_base_leakfree(hard, score, pred_ret_h1, thr, mag):
    """Mag gate using MODEL-predicted |return| — no lookahead."""
    gated = hard.copy()
    keep = (score >= thr) & (np.abs(pred_ret_h1) >= mag)
    gated[~keep] = FLAT_CLASS
    return gated


def gate_base_leaky(hard, score, trh1, thr, mag):
    """Original leaky gate using TRUE future |return| — for comparison only."""
    gated = hard.copy()
...
def gate_hX_agree(hard_h1, score, mag_src, hard_hX, thr, mag):
    gated = gate_base_leakfree(hard_h1, score, mag_src, thr, mag)
...
def main():
    ...
    store = []
    print(f"\n=== Predicting panel ({MODEL_NAME}, idx_n={IDX_N}) ===", flush=True)
    for sym in PANEL:
        df = load_csv(sym)
        ...
        # TTA-averaged logits and return predictions
        confs = {}
        tta_ret = []
        for h_idx in range(4):
            ...
            avg_ret = np.mean([np.asarray(per_lb_ret[i])[:, h_idx] for i in range(len(lookbacks))], axis=0)
            tta_ret.append(avg_ret)
        tta_ret = np.asarray(tta_ret).T  # [N, 4]

        tdh1 = td[:, 0]
        trh1 = tr[:, 0]
        pred_ret_h1 = tta_ret[:, 0]  # model-predicted h1 return (leak-free mag source)
    ...
    # Leak-free configs (mag uses model |pred_ret_h1|)
    configs = [
        ("base", lambda s: gate_base_leakfree(s["hard"][0], s["score"], s["pred_ret_h1"], 0.45, GATE_MAG)),
        ("fade", lambda s: apply_fade(gate_base_leakfree(s["hard"][0], s["score"], s["pred_ret_h1"], 0.45, GATE_MAG), s["idx_arr"])),
        ("h3_agree_thr0.45", lambda s: gate_hX_agree(s["hard"][0], s["score"], s["pred_ret_h1"], s["hard"][1], 0.45, GATE_MAG)),
        ("h5_agree_thr0.45", lambda s: gate_hX_agree(s["hard"][0], s["score"], s["pred_ret_h1"], s["hard"][2], 0.45, GATE_MAG)),
        ("h3_selfade0.02", lambda s: apply_sel_fade(
            gate_hX_agree(s["hard"][0], s["score"], s["pred_ret_h1"], s["hard"][1], 0.45, GATE_MAG),
            s["idx_arr"], 0.02)),
    ]

    print(f"\n=== Leak-free vs leaky (r5 {MODEL_NAME}) ===", flush=True)
    print(f"  {'config':<22} {'lf ret':>8} {'lk ret':>8} {'lf beat':>8} {'lk beat':>8} {'lf prec':>9} {'lk prec':>9}", flush=True)
    for name, gate_fn in configs:
        r = eval_config(store, bh, gate_fn)
        results[name] = r
        lk = LEAKY_BASELINES.get(name, {})
        print(f"  {name:<22} {r['ret_mean']:8.3f} {lk.get('ret', 0):8.3f} "
              f"{r['beat_bh']:>4}/9   {lk.get('beat', 0):>4}/9   "
              f"{r['prec_mean']:8.2%} {lk.get('prec', 0):8.2%}", flush=True)

    # Also evaluate no-mag gate (pure confidence gate) as reference
    def gate_nomag(s):
        g = s["hard"][0].copy()
        g[s["score"] < 0.45] = FLAT_CLASS
        return g
    r_nm = eval_config(store, bh, gate_nomag)
    results["nomag_conf_only"] = r_nm
    print(f"\n=== Reference: no-mag (conf>=0.45 only) ===", flush=True)
    print(f"  ret={r_nm['ret_mean']:.3f} beat={r_nm['beat_bh']}/9 prec={r_nm['prec_mean']:.2%} cov={r_nm['cov_mean']:.2%}", flush=True)
    ...
    out_path = ROOT / "outputs" / "eval_p9_panel_r11_leakfix.json"
