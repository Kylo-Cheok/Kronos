"""Phase 9 R15 — Evaluate r20 (UP-DOWN swap penalty) leak-free on panel.

R14 confirmed full training data hurts (more aggressive = worse panel alpha).
R15 tests a structural loss change: swap penalty targeting UP<->DOWN confusion.
r20 config = r5 (frozen, pool48, cw=[2.0,0.5,2.0], 100k/epoch) + swap_penalty_weight=1.0.

Output: outputs/eval_p9_panel_r15_swap.json
"""
...
TEST_LO = "2025-12-15"
COST = 0.0005
MODELS = ["r5_frozen_pool48", "r20_frozen_swap"]
IDX_N = 5
...
def main():
    ...
    for model_name in MODELS:
        ...
        store = []
        print(f"=== Predicting panel ({model_name}) ===", flush=True)
        for sym in PANEL:
            df = load_csv(sym)
    ...
    if len(all_results) >= 2:
        ...
        print(f"\n=== {n2} vs {n1} (leak-free) ===", flush=True)
        print(f"  {'config':<22} {n2[:8]+' ret':>9} {n1[:8]+' ret':>9} {n2[:8]+' beat':>9} {n1[:8]+' beat':>9} {n2[:8]+' prec':>10} {n1[:8]+' prec':>10}", flush=True)
        for name in ["base", "fade", "h3_agree_thr0.45", "h5_agree_thr0.45", "h3_selfade0.02"]:
            r2 = all_results[n2].get(name, {})
            r1 = all_results[n1].get(name, {})
            print(f"  {name:<22} {r2.get('ret_mean',0):9.3f} {r1.get('ret_mean',0):9.3f} "
                  f"{r2.get('beat_bh',0):>6}/9   {r1.get('beat_bh',0):>6}/9   "
                  f"{r2.get('prec_mean',0):9.2%} {r1.get('prec_mean',0):9.2%}", flush=True)

    out_path = ROOT / "outputs" / "eval_p9_panel_r15_swap.json"
