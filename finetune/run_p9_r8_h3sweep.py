"""Phase 9 R8 — h3_agree conf sweep + selective fade.

R7 findings:
  - h3_agree: ret 0.074 beat 8/9 (best ret+beat balance)
  - fade: prec 42.10% (best prec)
  - h3+fade: prec 44.27% but ret drops to 0.055 (too strict)

R8 hypothesis: selective fade — only filter UP calls when index is STRONGLY up
(idx_ret > pos_thr), not when idx_ret >= 0. This keeps h3-confirmed calls in
mild up-markets (recovering 000333-like bounces) while filtering strong up-markets
where model has no alpha.

Sweep:
  1. h3_agree conf sweep: thr ∈ {0.30, 0.35, 0.40, 0.45, 0.50}
  2. h3_agree + selective fade: thr=0.45, pos_thr ∈ {0, 0.005, 0.01, 0.02}
  3. h5_agree conf sweep: thr ∈ {0.35, 0.40, 0.45}

Output: outputs/eval_p9_panel_r8_h3sweep.json
"""
...
def apply_selective_fade(g, idx_arr, pos_thr):
    """Filter UP only when index STRONGLY up (idx_ret > pos_thr).
    pos_thr=0 → traditional fade. pos_thr=0.01 → only filter when idx 5d ret > 1%."""
    g = g.copy()
...
def main():
    ...
    # 1. h3_agree conf sweep
    print("\n=== h3_agree conf sweep ===", flush=True)
    for thr in (0.30, 0.35, 0.40, 0.45, 0.50):
        r = eval_config(store, bh,
            lambda s, t=thr: gate_hX_agree(s["hard"][0], s["score"], s["trh1"], s["hard"][1], t, GATE_MAG))
        results[f"h3_agree_thr{thr}"] = r
        print(f"  thr={thr:.2f}: ret {r['ret_mean']:.3f} beat {r['beat_bh']}/9 "
              f"prec {r['prec_mean']:.2%} cov {r['cov_mean']:.2%}", flush=True)

    # 2. h5_agree conf sweep
    print("\n=== h5_agree conf sweep ===", flush=True)
    for thr in (0.35, 0.40, 0.45):
        r = eval_config(store, bh,
            lambda s, t=thr: gate_hX_agree(s["hard"][0], s["score"], s["trh1"], s["hard"][2], t, GATE_MAG))
        results[f"h5_agree_thr{thr}"] = r
        print(f"  thr={thr:.2f}: ret {r['ret_mean']:.3f} beat {r['beat_bh']}/9 "
              f"prec {r['prec_mean']:.2%} cov {r['cov_mean']:.2%}", flush=True)

    # 3. h3_agree (thr=0.45) + selective fade sweep
    print("\n=== h3_agree + selective fade sweep (thr=0.45) ===", flush=True)
    for pos_thr in (0.0, 0.005, 0.01, 0.02):
        r = eval_config(store, bh,
            lambda s, p=pos_thr: apply_selective_fade(
                gate_hX_agree(s["hard"][0], s["score"], s["trh1"], s["hard"][1], 0.45, GATE_MAG),
                s["idx_arr"], p))
        results[f"h3_sel_fade_pos{pos_thr}"] = r
        print(f"  pos_thr={pos_thr:.3f}: ret {r['ret_mean']:.3f} beat {r['beat_bh']}/9 "
              f"prec {r['prec_mean']:.2%} cov {r['cov_mean']:.2%}", flush=True)

    # 4. h3_agree (thr=0.40) + selective fade sweep
    print("\n=== h3_agree + selective fade sweep (thr=0.40) ===", flush=True)
    for pos_thr in (0.0, 0.005, 0.01, 0.02):
        r = eval_config(store, bh,
            lambda s, p=pos_thr: apply_selective_fade(
                gate_hX_agree(s["hard"][0], s["score"], s["trh1"], s["hard"][1], 0.40, GATE_MAG),
                s["idx_arr"], p))
        results[f"h3_thr0.40_sel_fade_pos{pos_thr}"] = r
        print(f"  pos_thr={pos_thr:.3f}: ret {r['ret_mean']:.3f} beat {r['beat_bh']}/9 "
              f"prec {r['prec_mean']:.2%} cov {r['cov_mean']:.2%}", flush=True)
    ...
    # Per-symbol for best prec (with cov > 10%)
    valid_prec = {k: v for k, v in results.items() if v["cov_mean"] > 0.10}
    best_prec = max(valid_prec.values(), key=lambda x: x["prec_mean"])
    best_prec_name = [k for k, v in valid_prec.items() if v is best_prec][0]
    print(f"\n=== Best prec (cov>10%): {best_prec_name} ===", flush=True)
    for ps in best_prec["per_symbol"]:
        print(f"  {ps['symbol']}: ret={ps['ret']:.3f} prec={ps['prec']:.2%} "
              f"cov={ps['cov']:.2%} n_calls={ps['n_calls']}", flush=True)

    out_path = ROOT / "outputs" / "eval_p9_panel_r8_h3sweep.json"
