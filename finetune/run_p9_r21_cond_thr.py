"""Phase 9 R21 — Regime-conditional threshold (no leakage).

R20 found within-symbol CSI300<0 → opt_thr=0.40, >=0 → 0.48 (opposite of
fade). This is oracle. R21 tests leak-free rule: CSI300 5d ret (known at
t) sets thr. If <0 → thr_lo, else thr_hi. Sweep (thr_lo, thr_hi) grid.

Compare vs fixed 0.45 and csi300_fade. No retraining; reuses R19 predict.

Output: outputs/eval_p9_panel_r21_cond_thr.json
"""
...
def gate_cond_thr(hard, score, idx_arr, thr_lo, thr_hi):
    """Leak-free: thr depends on CSI300 5d ret sign (known at t)."""
    g = hard.copy()
    dn = (idx_arr < 0) & ~np.isnan(idx_arr)
    up = (idx_arr >= 0) & ~np.isnan(idx_arr)
    nan = np.isnan(idx_arr)
    g[score < thr_lo] = FLAT_CLASS  # apply thr_lo to all first
    # then raise threshold for up-regime
    g[up & (score < thr_hi)] = FLAT_CLASS
    # nan regime: use conservative (higher) thr_hi
...
def main():
    ...
    for sym in PANEL:
        try:
            from evaluate_gated_ensemble import load_csv
            df = load_csv(sym)
    ...
    # Baselines
    configs = [("fixed_0.45", lambda s: gate_conf(s["hard"], s["score"], 0.45)),
               ("csi300_fade", lambda s: apply_fade(
                   gate_conf(s["hard"], s["score"], 0.45), s["idx_arr"]))]

    # Conditional thr grid: (thr_lo, thr_hi)
    grid = [(0.40, 0.48), (0.40, 0.50), (0.35, 0.48), (0.35, 0.50),
            (0.40, 0.45), (0.45, 0.50), (0.45, 0.55), (0.38, 0.48)]
    for tl, th in grid:
        name = f"cond_{tl:.2f}_{th:.2f}"
        configs.append((name, lambda s, tl=tl, th=th: gate_cond_thr(
            s["hard"], s["score"], s["idx_arr"], tl, th)))

    # Also: conditional thr + fade combo (down: low thr + fade up-regime UP)
    # Actually fade kills up-regime UP; cond thr lowers dn thr. Test combo:
    def combo_fade_cond(s, tl=0.40, th=0.48):
        g = gate_cond_thr(s["hard"], s["score"], s["idx_arr"], tl, th)
        return apply_fade(g, s["idx_arr"])
    configs.append(("fade+cond_0.40_0.48", lambda s: combo_fade_cond(s)))

    results = {}
    print(f"\n=== R21 configs ({n_sym} symbols) ===", flush=True)
    print(f"{'config':<22} {'ret_mean':>9} {'total':>8} {'beat':>7} {'prec':>7} {'cov':>7}", flush=True)
    for name, gate_fn in configs:
        r = eval_config(store, bh, gate_fn)
        results[name] = r
        print(f"{name:<22} {r['ret_mean']:>+9.3f} {r['total_ret']:>+8.3f} "
              f"{r['beat_bh']:>3}/{n_sym}  {r['prec_mean']:>6.2%} {r['cov_mean']:>6.2%}", flush=True)

    # Regime split for top configs
    ...
    print(f"{'config':<22} {'up_ret':>8} {'up_beat':>8} | {'dn_ret':>8} {'dn_beat':>8}", flush=True)
    for name, gate_fn in configs:
        ...
        print(f"{name:<22} {ret_up:>+8.3f} {beat_up:>3}/{len(up_syms)}  | "
              f"{ret_dn:>+8.3f} {beat_dn:>3}/{len(dn_syms)}", flush=True)

    out_path = ROOT / "outputs" / "eval_p9_panel_r21_cond_thr.json"
