"""Phase 9 R9 — focal loss model panel evaluation.

R8 established Pareto frontier with r5_frozen_pool48 (CE loss):
  h5_agree_thr0.45: ret 0.086 beat 7/9
  h3_agree_thr0.45: ret 0.074 beat 8/9
  h3+sel_fade(0.02): ret 0.078 beat 7/9 prec 41.46%

R9 trains r17_frozen_focal_g2 (same hyperparams as r5 but focal loss gamma=2.0).
This script evaluates r17 on the same panel with the same gate configs.

Output: outputs/eval_p9_panel_r9_focal.json
"""
...
def main():
    ...
    store = []
    print(f"=== Predicting panel with {MODEL} (idx_n={IDX_N}) ===", flush=True)
    for sym in PANEL:
        df = load_csv(sym)
    ...
    print("\n=== Config comparison (r17 focal vs r5 CE) ===", flush=True)
    print(f"  {'config':<22} {'r17 ret':>8} {'r5 ret':>8} {'r17 beat':>8} {'r5 beat':>8} {'r17 prec':>9} {'r5 prec':>9}", flush=True)
    ...
    out_path = ROOT / "outputs" / "eval_p9_panel_r9_focal.json"
